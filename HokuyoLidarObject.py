...
class HokuyoLidarObject(object):
    ...
    def processSample(self, sample, min_angle, max_angle, filter_threshold=5):
        points = self._convertDataToPoints(sample["data"], min_angle, max_angle)
        points = filter_points(points, filter_threshold)
        return points
        

    def _convertDataToPoints(self, data, min_angle, max_angle):
        points = []
        for index,r in enumerate(data):
            angle = self._mapf(index, 0, len(data)-1, min_angle, max_angle);
            x = r * rect_arithmetic.cosf( angle )
            y = r * rect_arithmetic.sinf( angle )
    ...
    def _mapf(self, x, in_min, in_max, out_min, out_max):
        return (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;

if __name__ == '__main__':
    device = HokuyoLidarObject('/dev/ttyACM0', 115200);
    points = device.processSample(device.getSample(), -135, 135, 5)
    xs,ys = zip(*[i.point for i in points])
    data = {"xs":xs, "ys":ys}
    corners = get_corners(points)
    #print "points :",json.dumps(data)
    device.close()
